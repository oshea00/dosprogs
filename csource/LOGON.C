 /***************************************************************************
 *
 *                    HLLAPI program using  C
 *
 ****************************************************************************
 *
 *     This program makes HLLAPI requests to logon to kgps
 *
 **************************************************************************/
...
char our_dca_logon[50] = "kgpstso(";
char our_dca_logon2[]  =  ") logmode(t3278m2)@E";
char our_password[11];
char our_userid[7];
...
void main(int argc, char *argv[])
{
	...
	if (argc == 1)
		...
		strcat(our_dca_logon,our_userid);
		strcat(our_dca_logon,our_dca_logon2);
		fflush(stdin);
	    }
	...
	i = wait_search_send( MAX_TRIES,
	    ...
	    our_dca_logon,  strlen(our_dca_logon) );
        if(i != 0 )
        {
	    printf( "Error 060: Logon failure return code= %d\n",i);
	    api_func = PCB_DISCONNECT;
	    hllc( &api_func, &api_null, &api_null, &api_null );
                exit(1);
        }
	 ...
	 * logon with password
	   ...
	   api_len = strlen(our_password);
	   api_retc = 0;
	   hllc(&api_func,our_password, &api_len, &api_retc);
	   if (api_retc != PCB_RC_SEND_KEY_OK )
	      {
		      printf( "Error 530: Send password failure, code %x\n", api_retc);
	      }
	};
	...
	printf( "Connected to KGPS with no errors.\n");
	printf( "Left and right <shift> simultaneously to toggle between sessions.\n");
        exit(1);
}
