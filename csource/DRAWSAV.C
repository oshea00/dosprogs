...
int main(void)
{
	...
	while (c!='e')
	{
		...
		if (c=='s'||c=='r'||c=='c'||c=='f'||c=='p'||c=='l'||c=='m'||c=='g'||c=='=')
			prompt("Cursor Mode.","Type 'e' to quit.","","");
	}
	c_break();
	return 0;
...
void manipulate_object(void)
{
	...
	prompt("Manipulate Object.","'q' to quit","'d' to delete","'m' to move");
	...
	switch (c)
	{
		case 'q' :  refresh();prompt("","","","");cursx=6;cursy=6;draw_cursor(ON);return;
		case 'd' :  delete_object(object,index);break;
		case 'm' :  move_object(object,index,line_end);break;
	}
	refresh();
	prompt("","","","");
	cursx=6;cursy=6;
...
void highlight_handle(int object,int index,int line_end)
{
	...
	switch (object)
	{
		case SQUARE  :  rectangle(squares[index].x,squares[index].y,squares[index].x+4,squares[index].y+4);
						floodfill(squares[index].x+1,squares[index].y+1,WHITE);break;
		case CIRCLE  :  rectangle(circles[index].x,circles[index].y,circles[index].x+4,circles[index].y+4);
						floodfill(circles[index].x+1,circles[index].y+1,WHITE);break;
		case FILEOBJ    :  rectangle(files[index].x,files[index].y,files[index].x+4,files[index].y+4);
						floodfill(files[index].x+1,files[index].y+1,WHITE);break;
		case ARROW   :  if (line_end == 1)
						{
							rectangle(arrows[index].x1,arrows[index].y1,arrows[index].x1+4,arrows[index].y1+4);
							floodfill(arrows[index].x1+1,arrows[index].y1+1,WHITE);
						}
						else
						{
							rectangle(arrows[index].x2,arrows[index].y2,arrows[index].x2+4,arrows[index].y2+4);
							floodfill(arrows[index].x2+1,arrows[index].y2+1,WHITE);
						}
						break;
		case LINE    :  if (line_end == 1)
						{
							rectangle(lines[index].x1,lines[index].y1,lines[index].x1+4,lines[index].y1+4);
							floodfill(lines[index].x1+1,lines[index].y1+1,WHITE);
						}
						else
						{
							rectangle(lines[index].x2,lines[index].y2,lines[index].x2+4,lines[index].y2+4);
							floodfill(lines[index].x2+1,lines[index].y2+1,WHITE);
						}
						break;
	}
}
...
void move_square(int index)
{
	int c=0;
	fflush(stdin);
	prompt("Move Mode.","Type 'e' to quit.","","");
	while (c!='e')
	{
		c = getch();
		switch(c)
		{
			case 'w' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,UP,SR);break;
			case 'W' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,UP,FR);break;
			case 'x' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,DOWN,SR);break;
			case 'X' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,DOWN,FR);break;
			case 'a' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,LEFT,SR);break;
			case 'A' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,LEFT,FR);break;
			case 'd' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,RIGHT,SR);break;
			case 'D' : disp_square(index,OFF,NONE,NONE);disp_square(index,ON,RIGHT,FR);break;
		}
	}
}
...
void move_circle(int index)
{
	...
	while (c!='e')
	{
		...
		switch(c)
		{
			case 'w' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,UP,SR);break;
			case 'W' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,UP,FR);break;
			case 'x' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,DOWN,SR);break;
			case 'X' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,DOWN,FR);break;
			case 'a' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,LEFT,SR);break;
			case 'A' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,LEFT,FR);break;
			case 'd' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,RIGHT,SR);break;
			case 'D' : disp_circle(index,OFF,NONE,NONE);disp_circle(index,ON,RIGHT,FR);break;
		}
	}
}
...
void move_file(int index)
{
	...
	while (c!='e')
	{
		...
		switch(c)
		{
			case 'w' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,UP,SR);break;
			case 'W' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,UP,FR);break;
			case 'x' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,DOWN,SR);break;
			case 'X' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,DOWN,FR);break;
			case 'a' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,LEFT,SR);break;
			case 'A' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,LEFT,FR);break;
			case 'd' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,RIGHT,SR);break;
			case 'D' : disp_file(index,OFF,NONE,NONE);disp_file(index,ON,RIGHT,FR);break;
		}
	}
}
...
void move_arrow(int index,int line_end)
{
	...
	while (c!='e')
	{
		...
		switch(c)
		{
			case 'w' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,UP,SR,line_end);break;
			case 'W' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,UP,FR,line_end);break;
			case 'x' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,DOWN,SR,line_end);break;
			case 'X' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,DOWN,FR,line_end);break;
			case 'a' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,LEFT,SR,line_end);break;
			case 'A' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,LEFT,FR,line_end);break;
			case 'd' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,RIGHT,SR,line_end);break;
			case 'D' : disp_arrow(index,OFF,NONE,NONE,NONE);disp_arrow(index,ON,RIGHT,FR,line_end);break;
		}
	}
}
...
void move_line(int index,int line_end)
{
	...
	while (c!='e')
	{
		...
		switch(c)
		{
			case 'w' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,UP,SR,line_end);break;
			case 'W' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,UP,FR,line_end);break;
			case 'x' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,DOWN,SR,line_end);break;
			case 'X' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,DOWN,FR,line_end);break;
			case 'a' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,LEFT,SR,line_end);break;
			case 'A' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,LEFT,FR,line_end);break;
			case 'd' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,RIGHT,SR,line_end);break;
			case 'D' : disp_line(index,OFF,NONE,NONE,NONE);disp_line(index,ON,RIGHT,FR,line_end);break;
		}
	}
}
...
void disp_square(int index,int toggle,int dir,int speed)
{
	...
	settextstyle(SMALL_FONT,0,0);
	setusercharsize(1,1,1,1);
	outtextxy(cursx+2,cursy+3,&squares[index].text[0][0]);
	outtextxy(cursx+2,cursy+12,&squares[index].text[1][0]);
	outtextxy(cursx+2,cursy+21,&squares[index].text[2][0]);
	outtextxy(cursx+2,cursy+30,&squares[index].text[3][0]);
}
...
void disp_circle(int index,int toggle,int dir,int speed)
{
	...
	settextstyle(SMALL_FONT,0,0);
	setusercharsize(1,1,1,1);
	outtextxy(cursx-27,cursy-20,&circles[index].text[0][0]);
	outtextxy(cursx-27,cursy-11,&circles[index].text[1][0]);
	outtextxy(cursx-27,cursy-2,&circles[index].text[2][0]);
	outtextxy(cursx-27,cursy+7,&circles[index].text[3][0]);
}
...
void disp_file(int index,int toggle,int dir,int speed)
{
	...
	settextstyle(SMALL_FONT,0,0);
	setusercharsize(1,1,1,1);
	outtextxy(cursx+2,cursy+3,&files[index].text[0][0]);
}
...
void disp_arrow(int index,int toggle,int dir,int speed,int line_end)
{
	...
	line(arrows[index].x1,arrows[index].y1,arrows[index].x2,arrows[index].y2);
	draw_point(arrows[index].x1,arrows[index].y1,arrows[index].x2,arrows[index].y2);
	settextstyle(SMALL_FONT,0,0);
	setusercharsize(1,1,1,1);
	outtextxy(arrows[index].tx,arrows[index].ty,&arrows[index].text[0][0]);
	outtextxy(arrows[index].tx,arrows[index].ty+9,&arrows[index].text[1][0]);
}
...
void disp_line(int index,int toggle,int dir,int speed,int line_end)
{
	...
	settextstyle(SMALL_FONT,0,0);
	setusercharsize(1,1,1,1);
	outtextxy(lines[index].tx,lines[index].ty,&lines[index].text[0][0]);
	outtextxy(lines[index].tx,lines[index].ty+9,&lines[index].text[1][0]);
}
...
void get_save(void)
{
	int c=0;
	fflush(stdin);
	prompt("Get/Save Mode.","Type 'e' to quit.","'s' to save","'g' to get");
	c = getch();
	switch (c)
	{
			case 's' : save_drawing();break;
			case 'g' : get_drawing();break;
	}
}
...
{
	char c=0;
	prompt("Use cursor commands","to move window","'e' to exit","");
	while (c!='e')
	{
		c = getch();
		switch(c)
