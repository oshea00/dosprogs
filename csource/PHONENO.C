...
{ {2,2,7,"search Names"},
  {2,3,7,"search Businesses"} };

char quicksub1[10] = "NB";
...
contact_entry_type contactlist[CONTACTSIZE];
contact_entry_type contact;
...
void clear_record(contact_entry_type *);
void getstrfld(int,int,int,int,int);
void fill_record(void);
void display_record(contact_entry_type *);
...
int mainmenu(void)
{
	...
	switch (choice)
	{
		...
		case  3  :  if (currentry>=0)
					{
						memcpy(&contact,&contactlist[currentry],sizeof(contact_entry_type));
						cursoron();
		...
		case  4  :  if (entrycnt>0&&currentry>=0)
					{
						...
						{
							memcpy(&contactlist[currentry],&contactlist[entrycnt-1],sizeof(contact_entry_type));
							entrycnt--;
							clear_fields();
						}
					};break;
...
void drawmainmenu(void)
{
	...
	textbackground(BLACK);
	textcolor(CYAN);
	border(1,1,80,25," Phone and Address List ");
	textcolor(LIGHTCYAN);
	textbackground(BLACK);
	...
	textbackground(BLACK);
	placestr(10,9,"    Name: ");
	placestr(10,10,"Business: ");
	placestr(10,11," Address: ");
	placestr(10,13,"    City: ");
	placestr(10,14,"   State: ");
	placestr(10,15,"     Zip: ");
	placestr(10,17,"   Phone: ");
	placestr(20,17,"(");
	placestr(24,17,")");
	placestr(29,17,"-");
	placestr(10,18,"    FAX#: ");
	...
	while (ascii!=ENTER)
	{
		...
		switch (keycode)
		{
			...
			case RIGHT : textcolor(orgfore);
						...
						putch(mainmenulst[i].choice[mainmenulst[i].highlight]);
						textcolor(selfore);break;
			case F1  :  spawnl(P_WAIT,"help","help.exe","phoneno.hlp",NULL);break;
...
void update_record(int recno)
{
	...
	{
		memcpy(&contactlist[recno],&contact,sizeof(contact_entry_type));
		activefile=1;
...
void create_record(void)
{
	...
	{
		memcpy(&contactlist[entrycnt],&contact,sizeof(contact_entry_type));
		entrycnt++;
		activefile=1;
...
int find_record(int keytype)
{
	...
	while(1)
	{
		while (i<entrycnt)
		{
			if (keytype==1)
			{
				if (compare(contactlist[i].name,input))
				{
					display_record(&contactlist[i]);
					currentry=i;
			...
			if (keytype==2)
			{
				if (compare(contactlist[i].business,input))
				{
					display_record(&contactlist[i]);
					currentry=i;
					if (messagewait("Keep looking?")==1)
...
void fill_record(void)
{
	getstrfld(20,9,25,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.name,input);
	getstrfld(20,10,15,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.business,input);
	getstrfld(20,11,25,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.address1,input);
	getstrfld(20,12,25,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.address2,input);
	getstrfld(20,13,15,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.city,input);
	getstrfld(20,14,2,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.state,input);
	getstrfld(20,15,5,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.zip,input);
	getstrfld(21,17,3,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.area,input);
	getstrfld(26,17,3,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.prefix,input);
	getstrfld(30,17,4,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.rest,input);
	getstrfld(21,18,3,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.fax_area,input);
	getstrfld(26,18,3,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.fax_prefix,input);
	getstrfld(30,18,4,BLUE,LIGHTCYAN);
	if (strlen(input)>0)
		strcpy(contact.fax_rest,input);
}

void clear_record(contact_entry_type *contact)
{
	contact->name[0]=NULL;
	contact->business[0]=NULL;
	contact->address1[0]=NULL;
	contact->address2[0]=NULL;
	contact->city[0]=NULL;
	contact->state[0]=NULL;
	contact->zip[0]=NULL;
	contact->area[0]=NULL;
	contact->prefix[0]=NULL;
	contact->rest[0]=NULL;
	contact->fax_area[0]=NULL;
	contact->fax_prefix[0]=NULL;
	contact->fax_rest[0]=NULL;
}

void display_record(contact_entry_type *contact)
{
	clear_fields();
	textcolor(LIGHTCYAN);
	placestr(20,9,contact->name);
	placestr(20,10,contact->business);
	placestr(20,11,contact->address1);
	placestr(20,12,contact->address2);
	placestr(20,13,contact->city);
	placestr(20,14,contact->state);
	placestr(20,15,contact->zip);
	placestr(21,17,contact->area);
	placestr(26,17,contact->prefix);
	placestr(30,17,contact->rest);
	placestr(21,18,contact->fax_area);
	placestr(26,18,contact->fax_prefix);
	placestr(30,18,contact->fax_rest);
}
	...
	if (strlen(selected_file))
	{
		if (editfilename())
		{
			...
			else
			{
				...
				if (!strcmpi(fileheader.headername,"phoneno"))
				{
					while (fread(&contactlist[i],sizeof(contact_entry_type),1,infile))
					{
						i++;
					}
					entrycnt=i++;
...
void save_file(void)
{
	int i=0;
	FILE *outfile;
	strcpy(fileheader.headername,"phoneno");
	clear_fields();
	fflush(outfile);
	if (strlen(selected_file)&&entrycnt>0)
	{
		...
		else
		{
			...
			{
				fwrite(&contactlist[i],sizeof(contact_entry_type),1,outfile);
			}
			fclose(outfile);
			entrycnt=0;
			placestr(65,25,"             ");
